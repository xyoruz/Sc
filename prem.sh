...
print_ok "Virtualisasi: $(systemd-detect-virt 2>/dev/null || echo 'unknown')"
...
install_nginx() {
    ...
    mkdir -p "$NGINX_WEB" 2>/dev/null || mkdir -p /var/www/html
...
install_haproxy() {
    ...
    print_ok "HAProxy terinstall (belum diaktifkan)"
}
...
# ---------- Install UDP Custom (BadVPN) ----------
install_udp() {
    print_step "Install UDP Custom (BadVPN)"
    # Disederhanakan: kalau butuh lengkap, buat script terpisah
    print_ok "UDP custom: skip (opsional)"
}
...
make_folders() {
    ...
    mkdir -p /etc/xyr/{db,quota/{vmess,vless,trojan,ss,ssh},limit/{vmess,vless,trojan,ss,ssh},usage/{vmess,vless,trojan,ss,ssh},locked/{vmess,vless,trojan,ss,ssh}}
    mkdir -p /var/log/xyr /var/backups/xyr /var/www/html

    touch /etc/xyr/db/{vmess.db,vless.db,trojan.db,shadowsocks.db,ssh.db}
    echo "OFF" > /etc/xyr/autolock

    print_ok "Folder & file data siap"
}

# ---------- Download Library dari GitHub ----------
download_lib() {
    print_step "Download Library (config, common, validate)"

    local base="/usr/local/sbin/xyr/lib"
    local files=("config.sh" "common.sh" "validate.sh")
...
# ---------- Download Semua File Executable ----------
download_menu() {
    print_step "Download Menu & Tools"

    local files_menu=(
        "menu/menu"
        "menu/m-ssh" "menu/m-vmess" "menu/m-vless" "menu/m-trojan" "menu/m-ss"
        "menu/m-sshws" "menu/m-noob" "menu/m-trial" "menu/m-bot"
    ...
    for path in "${all_files[@]}"; do
        ...
        if safe_dl "${REPO}/${path}" "/usr/local/sbin/${name}"; then
            ...
            echo -e "${C_YELLOW}SKIP (tidak ada)${C_RESET}"
        fi
    done
...
download_config() {
    ...
    for path in "${files[@]}"; do
        local name
        name=$(basename "$path")
        safe_dl "${REPO}/${path}" "/tmp/${name}" && \
            print_ok "Downloaded: $name" || print_err "Skip: $name"
    done
}
...
# ---------- Systemd Override (Stabil 24 Jam) ----------
systemd_tuning() {
    print_step "Systemd Tuning (Restart=always, LimitNOFILE)"

    local services=("xray" "nginx" "haproxy" "dropbear" "ssh" "sshd")
...
final_info() {
    ...
    echo ""
    echo -e " IP VPS   : $IP"
    echo -e " Domain   : belum di-set (gunakan menu AddHost)"
    echo -e " SSH      : 22, 143, 109"
    echo -e " Nginx    : 80, 443"
    echo ""
